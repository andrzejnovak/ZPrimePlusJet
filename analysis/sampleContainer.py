...
#        self._trig_numer.RebinY(5)
        ...
        histos1d = {            
            ...
            'h_DDCvLtag_ak8_Hcc1': ["h_" + self._name + "_DDCvLtag_ak8_Hcc1", "; p_{T}-leading DDCvL-tag;", 40, 0, 1],
            'h_DDCvBtag_ak8_aftercut': ["h_" + self._name + "_DDCvBtag_ak8_aftercut", "; p_{T}-leading DDCvB-tag;", 40, 0, 1],
        ...
        if not self._minBranches:
            histos1d_ext = {
                ...
                'h_dbtag_ak8': ["h_" + self._name + "_dbtag_ak8", "; p_{T}-leading double b-tag;", 40, -1, 1],
		            'h_DDBvLtag_ak8': ["h_" + self._name + "_DDBvLtag_ak8", "; p_{T}-leading DDBvL-tag;", 40, 0, 1],
                'h_DDCvLtag_ak8': ["h_" + self._name + "_DDCvLtag_ak8", "; p_{T}-leading DDCvL-tag;", 40, 0, 1],
                'h_DDCvBtag_ak8': ["h_" + self._name + "_DDCvBtag_ak8", "; p_{T}-leading DDCvB-tag;", 40, 0, 1],
                ...
                'h_n2b1sdddt_ak8_aftercut': ["h_" + self._name + "_n2b1sdddt_ak8_aftercut", "; p_{T}-leading N_{2}^{1,DDT};", 25, -0.5, 0.5],
		            'h_dbtag_ak8_aftercut': ["h_" + self._name + "_dbtag_ak8_aftercut", "; p_{T}-leading DDBvL-tag;", 40, 0, 1],
                ...
                'h_DDBvL_ak8_muCR4': ["h_" + self._name + "_DDBvL_ak8_muCR4", "; p_{T}-leading DDBvL-tag;", 40, 0, 1],
                'h_DDCvL_ak8_muCR4': ["h_" + self._name + "_DDCvL_ak8_muCR4", "; p_{T}-leading DDCvL-tag;", 40, 0, 1],
                'h_DDCvB_ak8_muCR4': ["h_" + self._name + "_DDCvB_ak8_muCR4", "; p_{T}-leading DDCvB-tag;", 40, 0, 1],
...
#        pt_binBoundaries = [450, 475, 500, 525, 550, 575, 600, 625, 650, 675, 700, 725, 750, 775, 800, 825, 850, 875, 1000, 1100, 1200, 1300, 1400, 1500]
        ...
        regions     = ['topR6_N2','QGquark','QGgluon', 'Hcc1']
        histos2d={}
        for r in regions:
            for pf in passfail:
    ...
    def loop(self):
		...
		for i in range(0, len(ptscale)):
            ...
            for syst in ['JESUp', 'JESDown', 'JERUp', 'JERDown']:
                ...
                if eval('jpt_8_%s' % syst) > PTCUTMUCR and jmsd_8 > MASSCUT and nmuLoose == 1 and neleLoose == 0 and ntau == 0 and vmuoLoose0_pt > MUONPTCUT and abs(
                        vmuoLoose0_eta) < 2.1 and isTightVJet and jtN2b1sdddt_8 < 0 and jDDCvB_8 > 0.2 and abs(
                                vmuoLoose0_phi - jphi_8) > 2. * ROOT.TMath.Pi() / 3. and n_MdR0p8_4 >= 1:
                    if jDDCvL_8 > self.DBTAGCUT:
                        (getattr(self, 'h_msd_ak8_muCR4_cvb_pass_%s' % syst)).Fill(jmsd_8, weight)
                    elif jDDCvL_8 > self.DBTAGCUTMIN:
                        (getattr(self, 'h_msd_ak8_muCR4_cvb_fail_%s' % syst)).Fill(jmsd_8, weight)
		...
		if (not self._minBranches): self.h_n2b1sdddt_ak8_aftercut.Fill(jtN2b1sdddt_8,weight)
            if jpt_8 > PTCUT and jmsd_8 > MASSCUT and met < METCUT and n_dR0p8_4 < NJETCUT and jtN2b1sdddt_8 < 0 and isTightVJet:
              self.h_DDCvBtag_ak8_aftercut.Fill(jDDCvB_8, weight)
              if jDDCvB_8 > 0.2:
              	if  rh_8<-2.1 and rh_8>-6.:
                  self.h_DDCvLtag_ak8_Hcc1.Fill(jDDCvL_8, weight)
            ...
            for syst in ['JESUp', 'JESDown', 'JERUp', 'JERDown']:
                ...
                if eval('jpt_8_%s' % syst) > PTCUT and jmsd_8 > MASSCUT and eval('met_%s' % syst) < METCUT and eval(
                                'n_dR0p8_4_%s' % syst) < NJETCUT and jDDCvB_8 > 0.2 and jtN2b1sdddt_8 < 0 and isTightVJet:
                    if jDDCvL_8 > self.DBTAGCUT:
                    ...
                    if jDDCvL_8 > self.DBTAGCUTMIN:
                        #(getattr(self, 'h_msd_ak8_Hcc1_fail_%s' % syst)).Fill(jmsd_8, weight)
                        (getattr(self, 'h_msd_v_pt_ak8_Hcc1_fail_%s' % syst)).Fill(jmsd_8, eval('jpt_8_%s' % syst),weight)
